...
const postProcessRenderedBody = (html = '') =>
  html
    .replace(/<p>\s*---\s*<\/p>/g, '<hr />')
...
const recoverMatterFallback = (source) => {
  ...
  if (secondFence === -1) return { data: {}, content: source };

  const frontmatter = source.slice(3, secondFence).trim();
  const content = source.slice(secondFence + 4);
  const data = {};
  const pick = (key) => {
...
const parseMatterSafe = (source, fileName) => {
  try {
    return matter(source);
  } catch (error) {
    console.warn(`[prerender] skipped invalid frontmatter file: ${fileName}`, error?.message || error);
    return recoverMatterFallback(source);
  }
};
...
const renderPage = ({ slug, title, description, date, authorName, keywords, image, content, faq, products }) => {
  ...
  const renderedBody = postProcessRenderedBody(markdownToHtml(sanitizedContent));
...
<head>
  ...
  <style>
    ...
    article { line-height: 1.8; background: #fff; border-radius: 14px; padding: 22px; box-shadow: 0 8px 20px rgba(15,23,42,0.07); }
    article h2 { margin-top: 30px; margin-bottom: 12px; font-size: 1.6rem; }
    article h3 { margin-top: 22px; margin-bottom: 10px; font-size: 1.3rem; }
    ...
    a { color: #0369a1; }
    a[target="_blank"][rel~="sponsored"] { display: inline-block; background: #0369a1; color: #fff; padding: 10px 18px; border-radius: 10px; text-decoration: none; font-weight: 700; }
    a[target="_blank"][rel~="sponsored"]:hover { background: #075985; }
    hr { border: 0; border-top: 1px solid #e2e8f0; margin: 26px 0; }
    .table-wrap { overflow-x: auto; margin: 16px 0; border: 1px solid #e2e8f0; border-radius: 10px; }
    .table-wrap table { margin: 0; }
    table { width: 100%; border-collapse: collapse; }
    th, td { border-bottom: 1px solid #e2e8f0; padding: 8px 10px; text-align: left; font-size: 0.95rem; }
...
const generatePrerenderPages = () => {
  ...
  for (const file of files) {
    ...
    const { data, content } = parseMatterSafe(source, file);
